 ...
 * on cannacoin testnet and won't be accepted by cannacoind running
 * on the main CCN blockchain (real CCN addresses start with D).
 */
...
async.waterfall([
    function(next) {
      ...
      var outputs = {};
      /* Leave one CCN unspent as transaction fee. */
    ...
    function(unsigned_hex, next) {
      ...
      cannacoin.signRawTransaction(unsigned_hex, [], [tx.private_key], next);
    },
    ...
    function(signed_hex, next) {
      ...
      cannacoin.decodeRawTransaction(signed_hex, next);
    }
