 ...
 *
 * Accepts a destination address and a CCN amount. A raw transaction is
...
console.log('Sending', amount, 'CCN to', destination_address);
...
async.waterfall([
  ...
  function(sum, txs, next) {
    ...
    cannacoin.getAccountaddress('change', function(err, change_address) {
      if (err) {
        next(err);
      } else {
  ...
  function(sum, txs, change_address, next) {
    ...
    cannacoin.createRawTransaction(inputs, outputs, next);
  },
  function(unsigned_hex, next) {
    console.log('Signing raw transaction...');
    cannacoin.signRawTransaction(unsigned_hex, next);
  },
  function(signed, next) {
    if (signed.complete) {
      console.log('\nYour offline transaction is:');
      console.log(signed.hex + '\n');
      console.log('Move this across your air gap and send it to the p2p network using the sendRawTransaction cannacoind command.\n');

      cannacoin.decodeRawTransaction(signed.hex, next);
    } else {
